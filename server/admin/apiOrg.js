...
router.get('/apiOrg', async (req, res) => {
  ...
  try {
    let tokens;
    const totalCount = await pool.query('SELECT COUNT(*) FROM api_tokens WHERE id <> 1');
...
router.put('/apiOrg/:id', async (req, res) => {
  const { id } = req.params;
  const { organization, expires_at, api_ids } = req.body; // include api_ids from body
...
// DELETE (Delete a token and update logs)
router.delete('/apiOrg/:id', async (req, res) => {
  ...
  try {
    ...
    console.error(`Error while trying to delete token with ID ${id}:`, err.message);
    res.status(500).send('Server error');
  }
});
