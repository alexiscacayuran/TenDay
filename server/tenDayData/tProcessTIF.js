...
const maskTif = async (targetFilePath, outputFileName, paramName) => {
  ...
  try {
    ...
    // Get scale from map
    const scale = scaleMap[paramName] || null;
    ...
    // Translate to COG
    const cog = await gdal.translateAsync(cogPath, tif, options);

    await cog.buildOverviewsAsync("AVERAGE", [2, 4, 8, 16, 32]);

    log(
      chalk.bgRedBright(`${originalFileName}`) +
        "→ COG saved to:" +
        chalk.bgRedBright(`${cogPath}`)
    );
        ...
        const processFiles = async (fileList, suffix) => {
          await Promise.all(
            fileList.map(async (resFile) => {
              ...
              const s3Key = `${year}${monthNumber}${String(day).padStart(
                2,
                "0"
              )}/${folderName}/${newFileName}.tif`;

              const filePath = path.join(folderPath, resFile);
              const result = await maskTif(filePath, newFileName, folderName);
              if (!result) return;
              ...
              if (fs.existsSync(cogPath)) {
                ...
                const uploadCOG = s3
                  ...
                  .catch((err) =>
                    console.error(
                      `Error uploading ${newFileName}.tif (COG) to S3: ${err}`
                    )
                  );
              ...
              if (fs.existsSync(maskedPath)) {
                ...
                const clipS3Key = `${year}${monthNumber}${String(day).padStart(
                  2,
                  "0"
                )}/${folderName}/${newFileName}_masked.tif`;

                const uploadMasked = s3
                  .send(
                    new PutObjectCommand({
                      Bucket: BUCKET_NAME,
                      Key: clipS3Key,
                  ...
                  .catch((err) =>
                    console.error(
                      `Error uploading ${newFileName}_masked.tif to S3: ${err}`
                    )
                  );
                ...
                console.log(
                  `Clipped file ${newFileName}_masked.tif not found.`
                );
              }
